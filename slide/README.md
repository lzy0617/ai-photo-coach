...
中文 Beamer，16:9，白底蓝灰配色，14 页（含最后的 Thank You）。推荐讲述 **约 10 分钟**（含 1 分钟左右的外部演示）；不要现场等待完整云端推理。
...
| --- | --- | --- | --- |
| 1 | 封面 | 0:25 | 一句话介绍端云分工 |
| 2 | 为什么做这个项目 | 0:40 | 拍摄时及时指导，拍摄后理解整张照片 |
| 3 | 系统怎么工作：香橙派 | 0:45 | 浏览器到香橙派的快速链路 |
| 4 | 系统怎么工作：云端 | 0:40 | 主动触发、SSH 转发和云端模型 |
| 5 | 拍摄时的实时建议 | 0:50 | 结合截图说明反馈闭环与四类指标 |
| 6 | 构图建议从哪里来 | 0:50 | bbox 如何变成具体建议 |
| 7 | Qwen3-VL 看懂整张照片 | 0:50 | 补充几何规则看不到的画面信息 |
| 8 | 代码 A | 0:40 | Letterbox 坐标还原与网页框对齐 |
| 9 | 代码 B | 0:40 | 归一化指标与评分权重 |
| 10 | 代码 C | 0:40 | 图像、指标和 JSON 输出约束 |
| 11 | 实际运行效果 | 1:15 | 实时指导 → 拍摄 → 诊断 → 深度分析 |
| 12 | 遇到的问题与解决办法 | 0:45 | 抓住请求堆积、云端降级等工程问题 |
| 13 | 做到了什么、下一步怎么改 | 0:45 | 完成项、当前限制和下一步 |
| 14 | Thank You | 0:15 | 一句话收束并邀请提问 |

演示建议：前 20 秒展示实时构图变化，再用 20 秒拍摄并进入照片诊断，最后 20 秒展示事先准备好的深度分析结果。外部操作控制在 1 分钟左右；若现场切换耗时，缩短演示或问题页讲述。完整逐页讲稿见 `docs/presentation-script-10min.md`。文稿没有虚构延迟、帧率、精度等测试数据。

## 代码出处

幻灯片采用项目代码的教学节选，已标注省略或合并换行，并非可独立执行的完整代码。

- A：`edge-backend/detector.py` 的 `AscendYoloDetector.detect()`；先从模型坐标中去掉 letterbox padding，再除以缩放比例还原原图坐标，最后生成供网页绘制使用的归一化 bbox。代码节选省略了 `x2`、`y2` 的重复边界裁剪与归一化写法。
